...
import com.sap.module.util.LogSource;
...
public class ZipfileWithPassword {
	...
	public void compressWithPassword(AuditLogHelper audit,
		...
		log.add(" --------- List file to zip ---------");//write log list file to zip
		//Archive file from List
		for(int i=0; i<listFileSrc.size(); i++){
			log.add(listFileSrc.get(i).getName()); //write log list file to zip
		...
		//Write log file backup src
		String FileNameLog = prefix_suffix_time.Soure_preFix_sufFix_timeStamp(paramConnection.getFileNameSrcLog(), audit, paramConnection, log, paramConnection.getFormatTimeStamp());
		logSource.WriteLogSrc(paramConnection.getSrcDirLog(), FileNameLog, log, audit);
		
		// Upload Zip File to FTPS
		Boolean isFinish = upload.UploadFTPS(audit, log, ftps, targetPath+ "/" + Target_preFix_sufFixFileZip+".zip", srcZip);
		
		// Archive Zip file
		if(isFinish == true){
		File archiveZipPath = new File(paramConnection.getSrcPath()+ "/archive/"+ Soure_preFix_sufFixFileZip+".zip");
